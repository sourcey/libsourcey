...
public:
	...
	TCPInitiator(int id = 0) : id(id), client(NULL), success(false) {}
	...
	void initiate(const Client::Options opts, const Net::IP& peerIP) 
	{
		...
		try	{
			client = new TCPClient(*this, opts);
...
protected:
	void onRelayStateChanged(TURN::Client& client, TURN::ClientState& state, const TURN::ClientState&) 
	{
	...
	void onClientConnectionCreated(TCPClient& client, Net::TCPClientSocket* sock, const Net::Address& peerAddr) //UInt32 connectionID, 
	{
	...
	void onClientConnectionState(TCPClient& client, Net::TCPClientSocket*, 
	...
	*/
	...
	void onConnectionBindError(TCPClient& client, UInt32 connectionID)
	{
		...
		if (_dataSocket) {
			_dataSocket->StateChanged -= delegate(this, &TCPInitiator::onDataSocketStateChanged);
	...
	void onDataSocketStateChanged(void* sender, Net::ClientState& state, const Net::ClientState&)
	{
