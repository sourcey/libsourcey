...
public:
	TCPAsyncConnector(/*Runner& runner, */TSocket& socket, const Address& peerAddress, int timeout = 0) : 
		...
		_timeout(timeout)
	{	
		Log("debug") << "[TCPAsyncConnector:" << this << "] Creating" << endl;
	...
	{
		Log("debug") << "[TCPAsyncConnector:" << this << "] Destroying" << endl;
	}
	...
	virtual void run()
	{
		...
		try {
			...
			Log("error") << "[TCPAsyncConnector:" << this << "] Connection Failed: " << exc.displayText() << endl;
		}
		delete this;
	}
