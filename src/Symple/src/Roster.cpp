...
Roster::Roster()
{	
	//Log("trace") << "[Roster:" << this << "] Creating" << endl;
}
	

Roster::~Roster() 
{
	//Log("trace") << "[Roster:" << this << "] Destroying" << endl;
}
...
{	
	...
	{
		FastMutex::ScopedLock lock(_mutex);
		Log("trace") << "[Roster:" << this << "] Getting Our Peer: " << _ourAddress.toString() << endl;
		...
		Log("trace") << "[Roster:" << this << "] Getting Our Peer: 1: " << _ourAddress.toString() << endl;
	}
...
void Roster::update(const JSON::Value& data, bool whiny)
{
	if (data.isObject() &&
		...
		data.isMember("name") && 
		data.isMember("type")) {
		Log("trace") << "[Roster:" << this << "] Updating: " << JSON::stringify(data, true) << endl;
...
Roster::~Roster() 
{
	//Log("trace") << "[Roster:" << this << "] Destroying" << endl;
	closeAll();
	Log("trace") << "[Roster:" << this << "] Destroying: OK" << endl;
}
...
{
	FastMutex::ScopedLock lock(_mutex);
	Log("trace") << "[Roster:" << this << "] Closing All Peers: " << _items.size() << endl;
	
	PeerMap::iterator it = _items.begin();
	PeerMap::iterator it2;
	while (it != _items.end()) {
		it2 = it++;
		Log("trace") << "[Roster:" << this << "] Closing: " << it->second->name() << endl;
...
bool Roster::addPeer(Peer* stream, bool whiny) 
{
	...
	Log("trace") << "[Roster:" << this << "] Adding Peer: " << stream->name() << endl;
...
bool Roster::removePeer(const string& name) 
{
	...
	Log("trace") << "[Roster:" << this << "] Removing Peer: " << name << endl;		
...
bool Roster::closePeer(const string& name, bool whiny) 
{
	...
	Log("trace") << "[Roster:" << this << "] Closing Peer: " << name << endl;
...
{
	Log("trace") << "[Roster:" << this << "] Peer State Changed: " << state.toString() << endl;
	...
	if (state.id() == PeerState::Closed) {
		...
		if (_freeClosedPeers) {
			Log("trace") << "[Roster:" << this << "] Peer State Changed: Freeing: " << stream->name() << endl;
			Roster::Manager::free(stream->name());
		} else {
			Log("trace") << "[Roster:" << this << "] Peer State Changed: Removing: " << stream->name() << endl;
