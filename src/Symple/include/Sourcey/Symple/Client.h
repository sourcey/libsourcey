...
// ---------------------------------------------------------------------
//
class IClient: public SocketIO::ISocket
...
public:
	...
	virtual ClientOptions& options() = 0;
...
// ---------------------------------------------------------------------
//
template <class SocketIOBaseT>
class ClientBase: public SocketIOBaseT
{
public:
	ClientBase(Net::Reactor& reactor, Runner& runner, const ClientOptions& options = ClientOptions()) : 
		SocketIOBaseT(reactor),
		_runner(runner),
		_options(options),
		_announceStatus(500)
	{
	...
	{
		assert(isOnline());
		return SocketIOBaseT::send(SocketIO::Packet::Message, data, false);
	}
	...
	{	
		...
		return SocketIOBaseT::send(message, false);
	}
...
protected:	
	virtual int announce()
	{
		...
		txn->StateChange += delegate(this, &ClientBase::onAnnounce);
		return txn->send();
	}
...
//typedef Symple::ClientBase<  > Client; //, Symple::IClient //Symple::SocketBase< ::TCPContext>
//typedef Symple::ClientBase< SocketIO::SocketBase< Net::StatefulSocketBase< Net::SocketBase< ::SSLContext, Symple::IClient> > > > SSLClient; //, Symple::IClient //Symple::SocketBase< ::SSLContext>
//SocketIO::SSLSocket
// Net::SocketBase< ::SSLContext, SocketIO::ISocket>

 //, public Symple::IClient
//class Client: public SocketIOBaseT
