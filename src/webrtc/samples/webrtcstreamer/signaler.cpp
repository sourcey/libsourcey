...
void Signaler::onPeerConnected(smpl::Peer& peer)
{
    ...
    // Thread here:
    // https://groups.google.com/forum/#!topic/discuss-webrtc/d1fSm8Ns0Ww
    ...
    //  - AudioSourceInterface->RegisterAudioObserver(AudioObserver* observer)
    //  // data comes from here? (only used in receiver code, not source?)
    ...
    //   - AudioRtpSender will track_->AddSink the LocalAudioSinkAdapter, but we
    //   need
    //     to override this and add out own LocalAudioSinkAdapter

    // TODO:
    // - Custom AudioTrackInterface and LocalAudioSinkAdapter
    // - Override AudioTrackInterface->AddSink and prevent webrtc setting
    // default LocalAudioSinkAdapter
    ...
    //   RTC_DCHECK(signaling_thread_->IsCurrent());
    //   rtc::scoped_refptr<AudioTrackInterface> track(AudioTrack::Create(id,
    //   source));
    ...
    //   return VideoTrackProxy::Create(signaling_thread_, worker_thread_,
    //   track);

    // static_cast<webrtc::PeerConnectionFactory*>(_factory.get())->signaling_thread();

    // rtc::scoped_refptr<webrtc::AudioTrackInterface>
    // audioTrack(webrtc::AudioTrackProxy::Create( //webrtc::
    ...
    //             _factory->CreateVideoSource(new VideoPacketSource(device.id),
    //             nullptr)));
