#include "Anionu/RTSP/H263VideoSource.h"
...
// ============================================================================
H263VideoSource*
H263VideoSource::createNew(UsageEnvironment& env, Anionu::Input::VideoStream* videoStream, int streamId /* = 0 */,
							    int fps /* = 25*/, int videoWidth /* = 352 */, int videoHeight /* = 288 */, int encoderBufferSize /*= 1000000*/) 
{
	return new H263VideoSource(env, videoStream, streamId, fps, videoWidth, videoHeight, encoderBufferSize);
}


H263VideoSource::H263VideoSource(UsageEnvironment& env, Anionu::Input::VideoStream* videoStream, int streamId,
...
H263VideoSource::~H263VideoSource()
{
	cout << "H263VideoSource::~H263VideoSource()" << endl;
	if (_videoEncoder)
...
void H263VideoSource::encodeFrame() 
{
}


void H263VideoSource::doGetNextFrame() {
	...
	*_videoStream >> frame;
	...
	if (_encodedFrameSize > _encoderBufferSize || _encodedFrame == NULL) {
		cout << "Error: the h263 frame to stream is too big or NULL. " << endl;
		return;
	}
		
    if (_encodedFrameSize <= 0 || (unsigned int)_encodedFrameSize > fMaxSize) {
		cout << "Error: the h263 frame to stream is too big. " << endl;
		return;
	}
...
void H263VideoSource::deliverFrame() {
	...
	if (fFrameSize >= fMaxSize) {
		cout << "H263VideoSource::doGetNextFrame(): " 
