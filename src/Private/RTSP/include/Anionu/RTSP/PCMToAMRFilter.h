#ifndef AMR_FROM_PCM_H
#define AMR_FROM_PCM_H
...
/**
 * A filter class that converts PCM packets into AMR packets.
 * Uses internally ffmpeg for this conversion.
 */
class PCMToAMRFilter : public FramedFilter
{
public:
	//
	// Static method.
	...
	//	
	static PCMToAMRFilter* createNew(UsageEnvironment& env, FramedSource* audioSource, 
									int bitRate = 64000, 
									int numChannels = 2, 
									int sampleRate = 44100);

	virtual ~PCMToAMRFilter ();
protected:
	//
	// Constructor.
	// @see createNew-
	//
	PCMToAMRFilter(UsageEnvironment& env, FramedSource* audioSource, 
									int bitRate = 64000, 
									int numChannels = 2, 
									int sampleRate = 44100);
...
private: // Functions
	...
	// @param clientData Usually a "this" pointer to PCMToAMRFilter.
...
private: // Data
	...
	// The size of each AMR frame: FrameLengthInBytes = 144 * BitRate / SampleRate + Padding */
	unsigned int _amrFrameSize;
};
