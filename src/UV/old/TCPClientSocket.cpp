...
TCPClientSocket::TCPClientSocket(uv_loop_t* loop) :
	TCPContext(loop)
{
	LogTrace("TCPClientSocket", this) << "Creating" << endl;
}

	
TCPClientSocket::~TCPClientSocket() 
{	
	LogTrace("TCPClientSocket", this) << "Destroying" << endl;
}
...
TCPClientSocket::TCPClientSocket(const Scy::Net::StreamSocket& socket, Reactor& reactor, bool destroyafterClose) :
	_impl(socket),
	_reactor(reactor),
	_destroyafterClose(destroyafterClose)
{
...
static void after_write(uv_write_t* req, int status) {
	...
	LogTrace("debug") << "after_writeafter_writeafter_write: " << status << endl;	
...
void OnRead(uv_stream_t* handle, ssize_t nread, uv_buf_t buf) {
  //OnReadCommon(handle, nread, buf, UV_UNKNOWN_HANDLE);
	...
	LogTrace("debug") << "OnReadOnReadOnRead: " << nread << endl;	
}


uv_buf_t onDefaultAlloc(Handle* handle, size_t size) {
  uv_buf_t buf;
...
/*
uv_buf_t onDefaultAlloc(Handle* handle, size_t suggested_size) {
	LogTrace("debug") << "onDefaultAlloc: " << suggested_size << endl;	
...
{
	LogTrace("debug") << "AfterConnectAfterConnect: " << status << endl;	
...
*/
  ...
  for (i = 0; i < WRITES; i++) {
	...
	r = uv_read_start((uv_stream_t*)tcp, onDefaultAlloc, OnRead);
	assert(r == 0);
	//}
...
{
	LogTrace("TCPClientSocket", this) << "Connecting to " << remoteAddr.toString() << endl;	
	...
	try {
		...
		if (r) {
			const char* err = uv_err_name(uv_last_error(uv_default_loop()));
			LogTrace("error") << "[TCPClientSocket:" << this << "] Connection failed: " << err << endl;
			setState(this, ClientState::Disconnected, err);
			throw Exception(err);
		} 
		
		LogTrace("info") << "[TCPClientSocket:" << this << "] EVENT LOOP RUNNING" << endl;
	...
	*/
	...
	/*
		...
		LogTrace("error") << "[TCPClientSocket:" << this << "] Connection failed: " << e.displayText() << endl;
		e.rethrow();
	}
}
...
{	
	...
	//	uv_buf_t bufs[], int bufcnt, uv_write_cb cb);uv_default_loop(), 
	...
	if (uv_write(write_req, _connection.handle, &_buf, 1, after_write)) {
		uv_err_t err = uv_last_error(uv_default_loop());
		LogTrace("error") << "[TCPClientSocket:" << this << "] Send Error: " << uv_strerror(err) << endl;
	}
	else
		assert(0);
...
void TCPClientSocket::close()
{
	LogTrace("TCPClientSocket", this) << "Closing" << endl;	
	bool destroy = false;
	{
		FastMutex::ScopedLock lock(_mutex);
...
{
	LogTrace("TCPClientSocket", this) << "On Readable" << endl;
	pNf->release();
	...
	try	{
		...
		Buffer* buffer = NULL;;
		{
			...
			if (_buffer.capacity() < size) {
				LogTrace("TCPClientSocket", this) << "Resizing buffer: " << size << endl;
				_buffer.resize(size);
			}
			size = _impl.receiveBytes(_buffer.bytes(), size);
			LogTrace("trace") << "[TCPClientSocket:" << this << "] RECV: " << size << ": "
				<< _impl.address().toString() << "<--" << _impl.peerAddress1().toString() << endl;
			buffer = &_buffer;
			_buffer.position(0);
			_buffer.size(size);
		...
		if (size) {	
			...
			LogTrace("TCPClientSocket", this) << "Received EOF" << endl;
			close();
		}
	}
	catch (Scy::Net::ConnectionResetException& exc) {
		LogTrace("error") << "[TCPClientSocket:" << this << "] Peer closed connection: " << exc.displayText() << endl;
...
{
	LogTrace("TCPClientSocket", this) << "On Writable: " << _impl.available() << endl;
	pNf->release();
...
{
	LogTrace("warn") << "[TCPClientSocket:" << this << "] On Error" << endl;
	pNf->release();
	if (!stateEquals(ClientState::Error)) {
...
{	
	...
	if (recvSize != _buffer.capacity()) {
		LogTrace("TCPClientSocket", this) << "Buffer Size: " << recvSize << endl;
...
Scy::Net::StreamSocket& TCPClientSocket::impl()
{
	FastMutex::ScopedLock lock(_mutex); 
	return _impl;
}
