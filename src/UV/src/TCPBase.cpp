...
void TCPBase::connect(const net::Address& peerAddress) 
{
	...
	if (r)
		setLastError(true);
	traceL("TCPBase", this) << "Connecting: OK" << endl;
}
...
void TCPBase::close()
{
	...
	traceL("TCPBase", this) << "Close: " << tcp << ": " << refCount() << endl;
	
	//assert(tcp->reqs_pending == 0);
	Stream::close();
}
...
{	
	...
	assert(initialized());
	assert(flags == 0 && "currently unused");
...
{
	return reinterpret_cast<uv_tcp_t*>(handle<uv_stream_t>())->socket;
}
...
void TCPBase::onConnect(int status)
{
	...
	//traceL("TCPBase", this) << "On Connected: Emit: " << Connect.refCount() << endl;
