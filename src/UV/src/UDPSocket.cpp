...
UDPSocket::UDPSocket(uv_loop_t* loop) :
	UV::Base(loop)
{
	LogTrace("debug") << "[UDPSocket:" << this << "] Creating" << endl;
...
UDPSocket::~UDPSocket()
{
	LogTrace("debug") << "[UDPSocket:" << this << "] Destroying" << endl;
}
...
{
	LogTrace("debug") << "[UDPSocket:" << this << "] Closing" << endl;	

	uv_close((uv_handle_t*)&_handle, afterClose);
...
{	
	LogTrace("debug") << "[UDPSocket:" << this << "] Binding on " << address << endl;
...
{
	_error = err;
	LogTrace("error") << "[UDPSocket:" << this << "] Error: " << uv_strerror(err) << endl;	
	//setState(ClientState::Disconnected, uv_strerror(err));
	//shutdown();
}


uv_loop_t* UDPSocket::loop()
{
	return _loop;
}
...
{	
	...
	LogTrace("debug") << "[UDPSocket: " << socket << "] Handle Recv: " << nread << endl;
...
void UDPSocket::onSend(uv_udp_send_t* req, int status) 
{
	...
	LogTrace("debug") << "[UDPSocket: " << socket << "] Send Callback: " << status << endl;
...
void UDPSocket::afterClose(uv_handle_t* handle) 
{
	UDPSocket* socket = reinterpret_cast<UDPSocket*>(handle->data);
	LogTrace("debug") << "[UDPSocket: " << socket << "] On Close" << endl;
}
...
void UDPSocket::afterRead(uv&_handle_t* handle, ssize_t nread, uv_buf_t buf) 
{
	...
	LogTrace("debug") << "[UDPSocket: " << socket << "] Read Callback: " << nread << endl;
}
...
void UDPSocket::onAcceptConnection(uv&_handle_t* handle, int status) {
	...
	LogTrace("debug") << "[UDPSocket: " << socket << "] On Connection: " << status << endl;
...
{
	_error = err;
	LogTrace("error") << "[UDPSocket:" << this << "] Socket Error: " << uv_strerror(err) << endl;	
	//setState(ClientState::Disconnected, uv_strerror(err));
	close();
}
...
*/
  ...
  */
	...
	LogTrace("debug") << "[UDPSocket: " << socket << "] On Connection 1: " << status << endl;

	
	LogTrace("debug") << "[UDPSocket: " << socket << "] On Connection 2: " << status << endl;
...
uv_buf_t UDPSocket::defaultAlloc(uv_handle_t* handle, size_t size) 
{
	uv_buf_t buf;
	...
	*/
	...
	/*
	LogTrace("debug") << "onConnectedonConnected: " << status << endl;	
 ...
 // assert(r == 0);
	...
	*/
	...
	//int uv_udp_write(uv_loop_t* loop, uv_write_t* req, uv_udp_t* handle,
	...
	*/
		...
		LogTrace("info") << "[UDPSocket:" << this << "] EVENT LOOP RUNNING" << endl;
	...
	/*
		...
		LogTrace("error") << "[UDPSocket:" << this << "] Connection failed: " << e.displayText() << endl;
		e.rethrow();
	}
	*/
...
UDPSocket::UDPSocket(const scy::Net::StreamSocket& socket, Reactor& reactor, bool destroyafterClose) :
	_impl(socket),
	_reactor(reactor),
	_destroyafterClose(destroyafterClose)
{
...
{
	scy::Net::StreamSocket& rImpl = r->impl();
	 rImpl = _impl;
	delete this;
	//r = NULL;
	r->bindEvents();
...
{
	LogTrace("debug") << "[UDPSocket:" << this << "] On Readable" << endl;
	pNf->release();
	...
	try	{
		...
		Buffer* buffer = NULL;;
		{
			...
			if (_buffer.capacity() < size) {
				LogTrace("debug") << "[UDPSocket:" << this << "] Resizing buffer: " << size << endl;
				_buffer.resize(size);
			}
			size = _impl.receiveBytes(_buffer.bytes(), size);
			LogTrace("trace") << "[UDPSocket:" << this << "] RECV: " << size << ": "
				<< _impl.address().toString() << "<--" << _impl.peerAddress().toString() << endl;
			buffer = &_buffer;
			_buffer.position(0);
			_buffer.size(size);
		...
		if (size) {	
			...
			LogTrace("debug") << "[UDPSocket:" << this << "] Received EOF" << endl;
			close();
		}
	}
	catch (scy::Net::ConnectionResetException& exc) {
		LogTrace("error") << "[UDPSocket:" << this << "] Peer closed connection: " << exc.displayText() << endl;
...
{
	LogTrace("debug") << "[UDPSocket:" << this << "] On Writable: " << _impl.available() << endl;
	pNf->release();
...
{
	LogTrace("warn") << "[UDPSocket:" << this << "] On Error" << endl;
	pNf->release();
	if (!stateEquals(ClientState::Error)) {
...
{	
	...
	if (recvSize != _buffer.capacity()) {
		LogTrace("debug") << "[UDPSocket:" << this << "] Buffer Size: " << recvSize << endl;
...
scy::Net::StreamSocket& UDPSocket::impl()
{
	FastMutex::ScopedLock lock(_mutex); 
	return _impl;
}
