...
#ifndef SCY_AV_AVInputReader_H
#define SCY_AV_AVInputReader_H
...
public:
    ...
    typedef std::shared_ptr<AVInputReader> Ptr;
...
//#ifdef LIBAVDEVICE_VERSION
    virtual void openVideoDevice(int deviceID, int width = 0, int height = 0, double framerate = 0);
    virtual void openVideoDevice(const std::string& device, int width = 0, int height = 0, double framerate = 0);
    virtual void openAudioDevice(int deviceId, int channels, int sampleRate);
    virtual void openAudioDevice(const std::string& device, int channels, int sampleRate);
//#endif
