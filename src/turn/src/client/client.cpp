...
{		
	...
	setState(this, ClientState::Failed, util::format("(%d) %s", (int)errorAttr->errorCode(), errorAttr->reason().c_str()));

	if (!closed()) {
		_observer.onAllocationFailed(*this, errorAttr->errorCode(), errorAttr->reason()); // may result in deletion
	}
}
