...
public:
    ...
    void
    onSocketRecv(const MutableBuffer& buffer,
                 const net::Address& peerAddress) // net::SocketPacket& packet)
    {
        std::string payload(bufferCast<const char*>(buffer), buffer.size());
        DebugS(this) << id << ": On recv: " << peerAddress << ": " << payload
                     << std::endl; /// Echo back to client
...
void sendLatencyCheck()
{
    ...
    payload.append(util::itostr(time::ticks()));    /// Send large packets to
test throttling
...
void start(const net::Address& relayedAddr)
{
    debugL() << "UDPResponder: " << id << ": Starting on: " << relayedAddr <<
endl;
    ...
    try    {
        ...
        socket->attach(packetSlot<UDPResponder, stun::Message>(this,
&UDPResponder::onSTUNPacketReceived, 101));
        socket->attach(packetSlot<UDPResponder, RawPacket>(this,
&UDPResponder::onRawPacketReceived, 102));
        socket->connect(relayedAddr);

        Timer::getDefault().start(TimerCallback<UDPResponder>(this,
&UDPResponder::onTimer, 2000, 2000));
    }
    catch (std::exception& exc) {
        Log("error") << "UDPResponder: " << id << ": ERROR: " << exc.what() <<
endl;
    }
}
...
void stop()
{
    Timer::getDefault().stop(TimerCallback<UDPResponder>(this,
&UDPResponder::onTimer));
    if (socket) {
        socket->detach(packetSlot<UDPResponder, stun::Message>(this,
&UDPResponder::onSTUNPacketReceived, 101));
        socket->detach(packetSlot<UDPResponder, RawPacket>(this,
&UDPResponder::onRawPacketReceived, 102));
        delete socket;
        socket = NULL;
    }
...
{
    debugL() << "UDPInitiator: " << id << ": Received Data: " <<
std::string(data, size) << endl;    /// echo it back...
    client.sendData(data, size, peerAddr);
}

void onPermissionsCreated(turn::Client& client, const turn::PermissionList&
permissions)
{
...
{
    debugL() << "UDPResponder: " << id << ": On Timer: Sending data to: " <<
relayedAddr << endl;
    assert(socket);
...
{
/// echoed from client...
    debugL() << "UDPResponder: " << id << ": Data Packet Received: " <<
packet.size() << endl;
}
...
void onSTUNPacketReceived(void* sender, stun::Message& message)
{
    ...
    debugL() << "UDPResponder: " << id << ": STUN Packet Received: " <<
...
{
    debugL() << "UDPResponder: " << id << ": Allocation Created: " <<
transaction << endl;
    client->sendCreatePermissionRequest(_peerIP);
}


virtual void onAllocationRemoving(turn::Client* client, turn::Client*
allocation)
{
    debugL() << "UDPResponder: " << id << ": Allocation Deleted: " <<
transaction << endl;
}


virtual void onPermissionsCreated(turn::Client* client, turn::Client*
allocation)
{
    debugL() << "UDPResponder: " << id << ": Permissions Created: " <<
transaction << endl;
}
...
{
    debugL() << "UDPResponder: " << id << ": Received Data: " <<
std::string(data, size) << endl;
}

virtual void onAllocationFailed(Client* client, int errorCode)
{
    debugL() << "UDPResponder: " << id << ": Allocation Failed: " << errorCode
<< endl;
}
